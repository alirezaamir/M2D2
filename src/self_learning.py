...
def borrow_seizure():
    pat_rand = np.random.randint(1,24)
    dataset = test_dataset(pat_rand)
...
def self_eval(model, test_patient):
    ...
    for filename in filenames:
        with open(filename, "rb") as pickle_file:
            ...
            mmd_predict = model.predict(x, batch_size = 1)
...
def self_train(test_patient):
    source_arch = 'vae_sup_chb_total'
    source_model = 'vae_total_v26'
    ...
    auc_array = []
    auc = self_eval(trained_model, test_patient)
