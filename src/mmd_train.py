...
def main():
    ...
    for test_patient in range(1,25):
        ...
        # val_data, val_label = dataset_training("train", test_patient, all_filenames)
        ...
        vae_mmd_model.compile(optimizer='rmsprop', loss='mse')
        vae_mmd_model.fit(x=[train_data, train_random], y=train_label, validation_data=([val_data, val_random], val_label),batch_size=1, epochs=50)
        ...
        for node in sessions.keys():   # Loop2: nodes in the dataset
            ...
            print("y true shape: {}".format(y.shape))
            # exit()
            # z_dict[node] = latent
            #
            ...
            #
            # t_diff = np.abs(middle_points - middle_points)
